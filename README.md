...
The initial version of this project had the ECS service and RDS database in private subnets and a NAT gateway to enable outbound internet traffic. While this was the most obvious solution for me at the time, it turned out to be unnecessarily complex and expensive (the cost of NAT gateways is very high compared to the rest of the services, especially considering that each availability zone requires its own one). I've re-structured the infrastructure to drop the private subnets and NAT gateways and moved the ECS service and RDS database into the public subnets. Security groups make sure that the ECS service is only accessible by the load balancer and the database is only accessible by the ECS service. The original infrastructure can be found in the `private_subnets_and_nat_gateways` branch.
